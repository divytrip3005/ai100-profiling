...
Standalone microbenchmark for Grouped-Query Attention (GQA) on Qualcomm AI100 (QAIC).

GQA:  out = out_proj( softmax(Q @ K.T / sqrt(head_dim)) @ V )
      Q projection: d_model -> n_heads * head_dim
      K projection: d_model -> n_kv_heads * head_dim   (n_kv_heads < n_heads)
      V projection: d_model -> n_kv_heads * head_dim
      O projection: n_heads * head_dim -> d_model
      K/V heads are shared across n_groups = n_heads // n_kv_heads query heads.

Pipeline:
  1. Instantiate GQA PyTorch module and build synthetic inputs
...
Example usage:
  # Llama-3 8B attention: d_model=4096, n_heads=32, n_kv_heads=8
  python gqa_microbenchmark.py --d-model 4096 --n-heads 32 --n-kv-heads 8
  ...
  # MHA (n_kv_heads == n_heads), MQA (n_kv_heads == 1)
  python gqa_microbenchmark.py --d-model 4096 --n-heads 32 --n-kv-heads 32  # MHA
  python gqa_microbenchmark.py --d-model 4096 --n-heads 32 --n-kv-heads 1   # MQA
"""
...
class GQA(nn.Module):
    ...
    def __init__(
        ...
        # Pre-compute causal mask as a constant buffer so ONNX export emits
        # a constant tensor rather than a dynamic Trilu op (unsupported by QAIC).
        if causal:
            mask = torch.tril(torch.ones(seq_len, seq_len, dtype=torch.bool))
            # additive mask: 0 for keep, -inf for block
            bias = torch.zeros(seq_len, seq_len)
            bias.masked_fill_(~mask, float("-inf"))
            self.register_buffer("causal_bias", bias.unsqueeze(0).unsqueeze(0))
        else:
            self.causal_bias = None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.causal_bias is not None:
            attn = attn + self.causal_bias[:, :, :T, :T]
...
class GQAFused(nn.Module):
    """GQA using F.scaled_dot_product_attention (fused SDPA).

    Replaces the explicit  Q@K.T → scale → add_bias → softmax → @V  sequence
    with a single fused ONNX subgraph.  The QAIC compiler can lower this into
    fewer HMX↔HVX engine transitions, reducing sync-stall overhead.

    The causal mask is passed as an additive bias (same pre-computed constant
    buffer trick as GQA) so ONNX export stays free of dynamic Trilu ops.
    """
    def __init__(
        self,
        d_model: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Expand K/V so SDPA sees matching head counts
        k = k.repeat_interleave(self.n_groups, dim=1)
        v = v.repeat_interleave(self.n_groups, dim=1)

        # Single fused op: scores, scale, mask-add, softmax, weighted-sum
        # attn_mask is the pre-computed additive bias (0 or -inf) — no Trilu
        attn_mask = self.causal_bias[:, :, :T, :T] if self.causal_bias is not None else None
...
def build_inputs(
    ...
    *,
    ...
    return {"x": torch.randn(batch_size, seq_len, d_model, dtype=dtype)}
...
def build_compile_command(
    ...
    if depth_first:
        cmd.append("-aic-enable-depth-first")
    return cmd
...
def parse_op_summary_log(qpc_dir: Path) -> list[dict[str, Any]]:
    logs = sorted(qpc_dir.glob("QAicGraph_*_op_summary_final.log"))
    if not logs:
        return []
    log_path = logs[-1]
    ...
    with open(log_path) as f:
        for line in f:
            line = line.strip()
            if not line or line.startswith("#") or line.lower().startswith("op"):
                continue
            parts = line.split()
            if len(parts) < 3:
                continue
    ...
    ops.sort(key=lambda x: -x["cycles"])
    return ops


def _gqa_flops(B: int, S: int, D: int, n_heads: int, n_kv_heads: int) -> int:
    head_dim = D // n_heads
    # Q proj: B*S*D*D, K proj: B*S*D*(n_kv_heads*head_dim), V proj same, O proj: B*S*D*D
    proj_flops = 2 * B * S * (
        D * D +                            # Q
    ...
    # Attention scores: Q @ K.T  (B, n_heads, S, head_dim) x (B, n_heads, head_dim, S)
    attn_score_flops = 2 * B * n_heads * S * S * head_dim
    # Weighted sum: attn @ V
    attn_av_flops    = 2 * B * n_heads * S * S * head_dim
    return proj_flops + attn_score_flops + attn_av_flops
...
def build_report_card(
    ...
    op_summary: Optional[list[dict[str, Any]]] = None,
) -> dict[str, Any]:
    D  = args.d_model
    NH = args.n_heads
    NK = args.n_kv_heads
    ...
    head_dim = D // NH
    flops    = _gqa_flops(B, S, D, NH, NK)

    # Weight bytes: Q(D*D) + K(D*NK*head_dim) + V(D*NK*head_dim) + O(D*D)
    w_bytes_fp16 = D * D + D * NK * head_dim + D * NK * head_dim + D * D
    weight_bytes = w_bytes_fp16 * (0.75 if mxfp6 else 2.0)
    input_bytes  = B * S * D * 2.0
    output_bytes = B * S * D * 2.0
    ...
    return {
        "shape":            f"GQA  d_model={D}  n_heads={NH}  n_kv_heads={NK}  head_dim={head_dim}",
...
def print_report_card(rc: dict[str, Any]) -> None:
    ...
    print(f"\n{SEP}")
    print(f"  KERNEL REPORT CARD  --  {rc['shape']}")
    print(f"  variant={rc['attn_variant']}  causal={rc['causal']}  mxfp6={rc['mxfp6']}")
    print(f"  input={rc['batch_seq']}  KV-cache ratio={rc['kv_cache_ratio']:.3f}")
    print(f"  FLOPs={rc['flops']:,}  Arith.Intensity={fmt(rc['arith_intensity'])} FLOPs/byte")
    print(SEP)
    ...
    op_sum = rc.get("op_summary", [])
    if op_sum:
        print(f"\n  {DIV}")
        print(f"  OP SUMMARY  (QAicGraph_*_op_summary_final.log, top ops by cycles)")
        print(f"  {DIV}")
        total_cyc = sum(o["cycles"] for o in op_sum) or 1
        print(f"  {'Op Name':<40} {'Op Type':<24} {'Cycles':>10}  {'%Total':>7}")
        print(f"  {'-'*40} {'-'*24} {'-'*10}  {'-'*7}")
        for o in op_sum[:20]:
            pct = 100.0 * o["cycles"] / total_cyc
            print(f"  {o['op_name']:<40} {o['op_type']:<24} {o['cycles']:>10,}  {pct:>6.1f}%")
        if len(op_sum) > 20:
            rest_cyc = sum(o["cycles"] for o in op_sum[20:])
            print(f"  {'... (' + str(len(op_sum)-20) + ' more ops)':<40} {'':24} {rest_cyc:>10,}  {100.0*rest_cyc/total_cyc:>6.1f}%")
    ...
    kv_ratio   = rc.get("kv_cache_ratio", 1.0)
    variant    = rc.get("attn_variant", "GQA")
    if "OVERHEAD" in bottleneck:
        ...
        print(f"  61 attention layers/token = {total_calls} calls/token.")
        ...
        print(f"  --> Run at actual LLM shapes (e.g. d_model=4096, seq_len=1) for decode numbers.")
    elif "BANDWIDTH" in bottleneck:
        ddr = rc.get("ddr_bw_gbs") or 0.0
        print(f"  Decode is weight-movement dominated ({ddr:.2f} GB/s DDR BW).")
        print(f"  {variant}: KV weight is {kv_ratio:.2f}x of Q weight -- KV cache savings apply at inference.")
        print(f"  mxfp6 gives ~2.67x weight size reduction -> ~2.67x decode speedup.")
        print(f"  --> Try: fewer cores, larger batch, merged QKV projection.")
    elif "COMPUTE" in bottleneck:
        print(f"  HMX is the bottleneck -- good utilization at this seq_len (prefill regime).")
        print(f"  {variant}: softmax + attention scores scale as O(S²), dominant at long context.")
        print(f"  Decode (seq_len=1) will be bandwidth-bound; prefill is compute-bound.")
        print(f"  --> Focus: tile size tuning, core count sweep, flash-attention style fusion.")
    print(f"\n{SEP}\n")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="GQA microbenchmark for Qualcomm AI100.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    # Model shape
    p.add_argument("--d-model",         type=int, default=4096,
                   help="Model dimension (input/output of attention block).")
    ...
    p.add_argument("--causal",          action="store_true",
                   help="Apply causal (lower-triangular) attention mask.")
    p.add_argument("--fused-attn",      action="store_true",
                   help="Use F.scaled_dot_product_attention (fused SDPA) instead of "
                        "explicit matmul+softmax+matmul. Reduces HMX↔HVX sync stalls.")
    # Batch / sequence
    ...
    p.add_argument("--depth-first",    action="store_true",
                   help="Add -aic-enable-depth-first to qaic-compile. "
                        "Completes each tile's full path (QK→softmax→AV) before "
                        "moving to the next, keeping intermediates in VTCM and "
                        "reducing DDR round-trips between ops.")
    # Hardware timing
...
def main() -> None:
    ...
    if args.n_heads % args.n_kv_heads != 0:
        raise ValueError(
            f"n_heads ({args.n_heads}) must be divisible by n_kv_heads ({args.n_kv_heads})"
        )
    if args.d_model % args.n_heads != 0:
        raise ValueError(
            f"d_model ({args.d_model}) must be divisible by n_heads ({args.n_heads})"
        )
    ...
    flops = _gqa_flops(args.batch_size, args.seq_len, args.d_model, args.n_heads, args.n_kv_heads)
    inference_meta["flops"] = flops

    # ── 3. ONNX export ────────────────────────────────────────────────────────
    onnx_path = args.artifact_dir / (
        f"gqa{'_fused' if args.fused_attn else ''}"
        f"_d{args.d_model}_nh{args.n_heads}_nkv{args.n_kv_heads}"
    ...
    if hw_results or runner_metrics:
        ...
        op_summary       = parse_op_summary_log(args.artifact_dir / "qpc") \
                           if args.run_perf else None
