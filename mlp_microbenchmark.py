...
Standalone microbenchmark for a Llama-style SwiGLU MLP on Qualcomm AI100 (QAIC).

MLP:  out = down_proj( silu(gate_proj(x)) * up_proj(x) )
      3 linear projections, no bias by default (--bias to enable).

Pipeline:
  1. Instantiate MLP PyTorch module and build synthetic inputs
...
Example usage:
  # Llama-1/2 7B MLP: dim=4096, hidden=11008
  python mlp_microbenchmark.py --dim 4096 --hidden-dim 11008
...
# ── Model ─────────────────────────────────────────────────────────────────────
class MLP(nn.Module):
    """Llama-style SwiGLU MLP: out = down_proj(silu(gate_proj(x)) * up_proj(x))"""
...
class MLPFused(nn.Module):
    """Fused SwiGLU MLP: gate+up merged into one Linear(dim, 2*hidden_dim).
    Single matmul replaces two separate gate/up matmuls, eliminating one
    inter-matmul sync barrier. The compiler sees one HMX op for both projections.
    out = down_proj(silu(gate) * up)  where [gate, up] = fused_proj(x).split(hidden_dim, -1)
    """
    def __init__(self, dim: int, hidden_dim: int, bias: bool = False):
        super().__init__()
        self.fused_proj = nn.Linear(dim, hidden_dim * 2, bias=bias)
        self.down_proj  = nn.Linear(hidden_dim, dim, bias=bias)
...
def build_inputs(
    ...
    *,
    ...
    return {"x": torch.randn(batch_size, seq_len, dim, dtype=dtype)}
...
def parse_op_summary_log(qpc_dir: Path) -> list[dict[str, Any]]:
    """Parse QAicGraph_*_op_summary_final.log from qpc_dir.

    Each data line has the form (space/tab separated):
      OpName  OpType  Cycles  ...optionally more fields...
    Returns a list of dicts sorted by cycles descending.
    Requires -debug-glow + -aic-op-stats-verbosity 2 in
    QAIC_COMPILER_OPTS_UNSUPPORTED during compile.
    """
    logs = sorted(qpc_dir.glob("QAicGraph_*_op_summary_final.log"))
    if not logs:
        return []
    # Use the last (most refined) log if multiple exist
    log_path = logs[-1]
...
def build_report_card(
    ...
    D, H  = args.dim, args.hidden_dim
    B, S  = args.batch_size, args.seq_len
    mxfp6 = not args.no_mxfp6
    # 3 matmuls: gate(D->H), up(D->H), down(H->D)
    flops = 2 * B * S * (D * H + D * H + H * D)

    # weight bytes: gate + up + down
    weight_bytes = (D * H + D * H + H * D) * (0.75 if mxfp6 else 2.0)
    ...
    return {
        "shape":            f"MLP  dim={D}  hidden={H}  bias={args.bias}",
        "batch_seq":        f"[{B}, {S}, {D}]",
        "mxfp6":            mxfp6,
        "flops":            flops,
...
def print_report_card(rc: dict[str, Any]) -> None:
    ...
    print(f"  input={rc['batch_seq']}  mxfp6={rc['mxfp6']}")
    ...
    if "OVERHEAD" in bottleneck:
        ...
        print(f"  61 MLP layers/token = {total_calls} calls/token.")
        ...
        print(f"  --> Run at actual LLM shapes (e.g. dim=4096, hidden=14336) for real numbers.")
        ...
        print(f"  3 matmuls per MLP layer; core imbalance ({fmt(imb, '.1f')}%) compounds.")
        print(f"  --> Try: fewer cores, larger batch, fused SwiGLU kernel.")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Llama SwiGLU MLP microbenchmark for Qualcomm AI100.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    # Model shape
    p.add_argument("--dim",            type=int, default=4096,
                   help="Model dimension (input/output of MLP).")
    p.add_argument("--hidden-dim",     type=int, default=11008,
                   help="MLP hidden dimension (gate/up projection output).")
    p.add_argument("--bias",           action="store_true",
                   help="Enable bias in all three linear projections.")
    p.add_argument("--fused",          action="store_true",
                   help="Use fused gate+up projection (single Linear(dim, 2*hidden_dim)). "
                        "Reduces gate/up from 2 separate HMX matmuls to 1, "
                        "eliminating one inter-matmul sync barrier.")
    # Batch / sequence
...
def main() -> None:
    ...
    if log_path:
        log_path.write_text("")
        vprint(f"=== run started ===  MLP dim={args.dim} hidden={args.hidden_dim} "
               f"bias={args.bias} [{args.batch_size},{args.seq_len}]")

    # ── 1. Build module and inputs ────────────────────────────────────────────
    if args.fused:
        module = MLPFused(dim=args.dim, hidden_dim=args.hidden_dim, bias=args.bias)
    else:
        module = MLP(dim=args.dim, hidden_dim=args.hidden_dim, bias=args.bias)
    module = module.half()
    module.eval()
    ...
    flops = 2 * args.batch_size * args.seq_len * (
        args.dim * args.hidden_dim +   # gate_proj
        args.dim * args.hidden_dim +   # up_proj
        args.hidden_dim * args.dim     # down_proj
    )
    inference_meta["flops"] = flops

    # ── 3. ONNX export ────────────────────────────────────────────────────────
    onnx_path = args.artifact_dir / (
        f"mlp_d{args.dim}_h{args.hidden_dim}"
        f"{'_fused' if args.fused else ''}"
