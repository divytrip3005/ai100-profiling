...
The 32-row chunk is broadcast from the loading core to all other cores via the on-chip VTCM bus. This repeats 8 times (8 chunks × 32 rows = 256 rows) to assemble the full input tile in VTCM on every core.
...
Each core loads a **different** slice of the weight matrix — core 0 covers output channels 0–31, core 1 covers 32–63, and so on.
...
- Input tile:  256 KB (256 rows × 4096 K)  — wait, this is 2048 KB
- Weight tile: 256 KB (4096 K × 32 outC)
...
**Duration:** 14.4 µs (first op — later ops take ~20 µs as pipeline stabilises)

---

## Step 5 — Pipelined loop over all 43 weight tiles

Steps 2–4 repeat 43 times. They are **pipelined** — while HMX computes tile N, DMA is already loading tile N+1 (double-buffering). Observed from the timeline:

```
ts=  5µs : DMA loads INPUT chunk (256KB)
ts= 67µs : DMA loads WEIGHT tile #1 (100KB)
ts= 71µs : MULTICAST input
ts= 85µs : DMA loads WEIGHT tile #2  ← prefetch while #1 dequants
ts=104µs : DMA loads WEIGHT tile #3
ts=105µs : DEQUANT tile #1 (100KB → 256KB)
ts=124µs : DMA loads WEIGHT tile #4
ts=147µs : HMX #1 fires  (dur=14µs)
ts=161µs : HMX #2 fires  (dur=21µs)
ts=164µs : DMA loads WEIGHT tile #5
...
ts=881µs : HMX #43 fires (last op)
```

After 43 HMX ops, core 0 has computed output for:
- All **256 rows** × all **688 output channels** assigned to this core
...
As HMX finishes each output tile [256 × 32] (128 KB), HVX converts it from D32 internal format back to fp16, then DMA writes it to DDR.
...
All 16 cores write their output slices simultaneously. The full `[512 × 11008]` output assembles in DDR.

---

## VTCM layout at peak

During Step 4, per core:
...
│  ─────────────────────────────────────────────   │
│  Visible total                        2500 KB    │
│                                                  │
│  Compiler overhead (double-buffering,            │
│  format conversion, multicast staging) 2753 KB   │
│  ─────────────────────────────────────────────   │
...
> The compiler never exceeds ~62% VTCM utilisation. The extra ~36%
> above the simple formula is consumed by double-buffering the next
> weight tile, D32 format conversion buffers, and multicast staging.
...
The µs column is summed across all 16 cores × all threads — much larger than 1029 µs because DMA + HVX dequant + HMX compute **overlap** via double-buffering.

Source: `opstats/*.trace.json` event durations bucketed by op type.
...
The actual time is close to the bandwidth limit but HMX being active 70% of the time means compute engines are genuinely busy — this shape sits at the **bandwidth ↔ compute crossover point**.
...
| Each HMX op | `[256×4096] × [4096×32]` | trace `aicconvolutiond32` attributes |
